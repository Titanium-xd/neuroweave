"""
scripts/tasks/run_campaign_t001.py — First real ABB experiment campaign.
Runs 10 architectures on T-001 for 5 seeds each and generates a table + plot.
"""

from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path

import numpy as np
import pandas as pd
import scipy.sparse as sp
import matplotlib.pyplot as plt

sys.path.insert(0, str(Path(__file__).parent.parent.parent))

from abb.tasks.t001 import T001Config, T001Dataset
from abb.tasks.evaluate import run_benchmark
from abb.models.base import AgentConfig
from abb.models.baselines import A0Random, A7MLP, A8RNN
from abb.models.bio_variants import A1Bio, A1Frozen, A1Bias, A1Edge
from abb.models.topology import _er_rewire, _config_rewire, A3ER, A3Config, A3Dense


def _load_malecns_adj(max_nodes: int = 150):
    n_paths = list(Path("data/raw/malecns_e2e_validation").glob("neurons_*.parquet"))
    e_paths = list(Path("data/raw/malecns_e2e_validation").glob("edges_*.parquet"))

    if n_paths and e_paths:
        neurons = pd.read_parquet(n_paths[0]).head(max_nodes)
        all_edges = pd.read_parquet(e_paths[0])
        body_ids = set(neurons["bodyId"])
        edges = all_edges[
            all_edges["bodyId_pre"].isin(body_ids) & all_edges["bodyId_post"].isin(body_ids)
        ]
        bid_to_idx = {int(b): i for i, b in enumerate(neurons["bodyId"])}
        rows = edges["bodyId_pre"].map(bid_to_idx).to_numpy(dtype=np.int32)
        cols = edges["bodyId_post"].map(bid_to_idx).to_numpy(dtype=np.int32)
        col = "signed_weight" if "signed_weight" in edges.columns else "weight"
        vals = edges[col].to_numpy(dtype=np.float32)
        n = len(neurons)
        adj = sp.coo_matrix((vals, (rows, cols)), shape=(n, n))
        adj.sum_duplicates()
        return adj, n
    else:
        raise FileNotFoundError("MaleCNS cache not found.")


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--epochs", type=int, default=40)
    parser.add_argument("--seeds", type=int, default=5)
    parser.add_argument("--save-dir", type=str, default="artifacts/campaign_1")
    args = parser.parse_args()
    
    out_dir = Path(args.save_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 64)
    print("  ABB FIRST REAL EXPERIMENT CAMPAIGN (T-001)")
    print("=" * 64)

    cfg = T001Config()
    ds = T001Dataset(cfg)
    seeds = [42 + i for i in range(args.seeds)]
    
    print("Loading MaleCNS reference graph...")
    ref_adj, n_nodes = _load_malecns_adj(max_nodes=150)
    
    print("Generating topology controls...")
    er_adj = _er_rewire(ref_adj, seed=42)
    config_adj = _config_rewire(ref_adj, seed=42)
    
    architectures = [
        "A0 Random", "A1-BIO", "A1-FROZEN", "A1-BIAS", "A1-EDGE",
        "A3-ER", "A3-CONFIG", "A3-DENSE", "A7-MLP", "A8-LSTM"
    ]
    
    all_results = {}
    
    def get_builder(arch: str):
        frozen_p = 0
        adj_used = None
        
        def builder(seed: int):
            if arch == "A0 Random":
                return A0Random.build(obs_dim=cfg.d_obs, act_dim=cfg.d_act, seed=seed)
            elif arch == "A7-MLP":
                return A7MLP.build(obs_dim=cfg.d_obs, act_dim=cfg.d_act, hidden_dim=128, n_layers=3, seed=seed)
            elif arch == "A8-LSTM":
                return A8RNN.build(obs_dim=cfg.d_obs, act_dim=cfg.d_act, hidden_dim=128, n_layers=2, cell_type="lstm", seed=seed)
            elif arch == "A1-BIO":
                c = AgentConfig("A1-BIO", obs_dim=cfg.d_obs, act_dim=cfg.d_act, n_layers=2, seed=seed, assumption_ids=())
                return A1Bio(c, ref_adj)
            elif arch == "A1-FROZEN":
                c = AgentConfig("A1-FROZEN", obs_dim=cfg.d_obs, act_dim=cfg.d_act, n_layers=2, seed=seed, assumption_ids=())
                return A1Frozen(c, ref_adj)
            elif arch == "A1-BIAS":
                c = AgentConfig("A1-BIAS", obs_dim=cfg.d_obs, act_dim=cfg.d_act, n_layers=2, seed=seed, assumption_ids=())
                return A1Bias(c, ref_adj)
            elif arch == "A1-EDGE":
                c = AgentConfig("A1-EDGE", obs_dim=cfg.d_obs, act_dim=cfg.d_act, n_layers=2, seed=seed, assumption_ids=())
                return A1Edge(c, ref_adj)
            elif arch == "A3-ER":
                c = AgentConfig("A3-ER", obs_dim=cfg.d_obs, act_dim=cfg.d_act, n_layers=2, seed=seed, assumption_ids=())
                return A3ER(c, er_adj)
            elif arch == "A3-CONFIG":
                c = AgentConfig("A3-CONFIG", obs_dim=cfg.d_obs, act_dim=cfg.d_act, n_layers=2, seed=seed, assumption_ids=())
                return A3Config(c, config_adj)
            elif arch == "A3-DENSE":
                c = AgentConfig("A3-DENSE", obs_dim=cfg.d_obs, act_dim=cfg.d_act, n_layers=2, seed=seed, assumption_ids=())
                return A3Dense(c, n_nodes=n_nodes)
            else:
                raise ValueError(f"Unknown arch {arch}")
                
        # Determine graph stats
        if "A1" in arch or "A3-ER" in arch or "A3-CONFIG" in arch:
            if "A3-ER" in arch: adj_used = er_adj
            elif "A3-CONFIG" in arch: adj_used = config_adj
            else: adj_used = ref_adj
            
            # Param freezing logic
            if arch == "A1-FROZEN":
                frozen_p = int(adj_used.nnz) * 2 + 150 * 2  # weights + biases (2 layers)
            elif arch == "A1-BIAS":
                frozen_p = int(adj_used.nnz) * 2  # weights frozen
            elif arch == "A1-EDGE":
                frozen_p = 150 * 2  # biases frozen
            else:
                frozen_p = 0
        
        return builder, frozen_p, adj_used

    for arch in architectures:
        builder, frozen_p, adj_used = get_builder(arch)
        res = run_benchmark(
            arch_id=arch,
            task_id="T-001",
            model_builder=builder,
            train_set=ds.train,
            dev_set=ds.dev,
            test_set=ds.test,
            seeds=seeds,
            n_epochs=args.epochs,
            frozen_params=frozen_p
        )
        if adj_used is not None:
            res["metadata"]["graph_nodes"] = adj_used.shape[0]
            res["metadata"]["graph_edges"] = adj_used.nnz
        elif arch == "A3-DENSE":
            res["metadata"]["graph_nodes"] = n_nodes
            res["metadata"]["graph_edges"] = n_nodes * n_nodes
        
        all_results[arch] = res
        
        with open(out_dir / f"{arch.replace(' ', '_')}.json", "w") as f:
            json.dump(res, f, indent=2)

    # Print summary table
    print("\n" + "=" * 80)
    print("  FIRST REAL EXPERIMENT CAMPAIGN SUMMARY")
    print("=" * 80)
    print(f"  {'Architecture':<15}  {'Test Acc':>15}  {'Params (T/F)':>15}  {'Edges':>10}  {'Time':>8}")
    print("  " + "─" * 72)
    for arch in architectures:
        r = all_results[arch]
        agg = r["aggregate"]
        meta = r["metadata"]
        
        acc = f"{agg['test_acc_mean']*100:.1f}% ± {agg['test_acc_ci95']*100:.1f}%"
        p_t = meta["trainable_parameters"]
        p_f = meta["frozen_parameters"]
        p_str = f"{p_t} / {p_f}"
        edges = meta.get("graph_edges", "-")
        t = f"{agg['wall_time_s_mean']:.1f}s"
        
        print(f"  {arch:<15}  {acc:>15}  {p_str:>15}  {edges:>10}  {t:>8}")
    print("=" * 80)
    
    # Plotting
    try:
        import matplotlib.pyplot as plt
        plt.style.use('seaborn-v0_8-whitegrid')
        
        fig, ax = plt.subplots(figsize=(10, 6))
        
        means = [all_results[a]["aggregate"]["test_acc_mean"] * 100 for a in architectures]
        cis = [all_results[a]["aggregate"]["test_acc_ci95"] * 100 for a in architectures]
        
        x = np.arange(len(architectures))
        
        # Color coding
        colors = []
        for a in architectures:
            if "A1" in a: colors.append("#2ca02c")  # Green for Bio
            elif "A3" in a: colors.append("#ff7f0e") # Orange for Topology controls
            elif "A0" in a: colors.append("#7f7f7f") # Gray for Random
            else: colors.append("#1f77b4")           # Blue for dense ML
            
        bars = ax.bar(x, means, yerr=cis, capsize=5, color=colors, alpha=0.8)
        
        ax.set_ylabel("Test Accuracy (%)")
        ax.set_title("T-001: Binary Pattern Discrimination (5 Seeds, 95% CI)")
        ax.set_xticks(x)
        ax.set_xticklabels(architectures, rotation=45, ha="right")
        ax.set_ylim(40, 105)
        ax.axhline(y=50, color='r', linestyle='--', alpha=0.5, label='Chance (50%)')
        
        # Custom legend
        from matplotlib.patches import Patch
        legend_elements = [
            Patch(facecolor='#2ca02c', alpha=0.8, label='Bio-Constrained (A1)'),
            Patch(facecolor='#ff7f0e', alpha=0.8, label='Topology Controls (A3)'),
            Patch(facecolor='#1f77b4', alpha=0.8, label='Dense ML Baselines (A7/A8)'),
            Patch(facecolor='#7f7f7f', alpha=0.8, label='Random (A0)')
        ]
        ax.legend(handles=legend_elements, loc='lower right')
        
        plt.tight_layout()
        plot_path = out_dir / "campaign_1_results.png"
        plt.savefig(plot_path, dpi=300)
        print(f"\nPlot saved to {plot_path}")
    except Exception as e:
        print(f"Could not generate plot: {e}")

if __name__ == "__main__":
    main()
